    ...
    <MainLayoutDashboard>
        <div class="p-6">
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
                ...
                <div v-else-if="!loading && !docentesFiltrados.length"
                    ...
                    <p v-else>No se encontraron docentes que coincidan con la búsqueda: <span
...
const fetchEnabledProfessors = async () => {
    loading.value = true;
    error.value = null;
    ...
    try {
        ...
        if (status === 404) {
            // No hay docentes activos, trátalo como lista vacía
            allDocentes.value = [];
            error.value = null;
        } else if (status === 401 || status === 403) {
            error.value = err.response?.data?.message || 'Acceso no autorizado. Verifica tu sesión.';
...
const fetchDisabledProfessors = async () => {
    loading.value = true;
    error.value = null;
...
const fetchSuspendedProfessors = async () => {
    loading.value = true;
    error.value = null;
...
const fetchByDepartment = async (department_id) => {
    loading.value = true;
    error.value = null;

    try {
        const response = await axios.post(
            `/api/users/get/department/by-role/`,
            {
                rol_nombre: 'docente',
...
const handleSelectFilter = () => {
    if (selectedOption.value === 'view-all' || selectedOption.value === '') {
        ...
        fetchEnabledProfessors();
    } else if (selectedOption.value === 'view-inactives') {
        // Filtrar docentes inactivos
        fetchDisabledProfessors();
